...
- **Fixed import data loss from PDFs (two distinct causes).**
  - *Extraction:* the chunked read path in `import_campaign.py` flattened all whitespace (`text.split()` / `" ".join()`), collapsing headings, indented read-aloud/boxed text, stat blocks, and tables into one undifferentiated wall of words. Since published adventures almost always exceed the 4000-word chunk threshold, the importer essentially always took this path — destroying the structural cues that mark important encounter details and causing them to be dropped. `build_chunks()` now splits only on line boundaries (and prefers section/encounter headings), preserving layout in every chunk while losing no words. Regression test added (`tests/test_import_chunking.py`).
  - *Storage + play:* even with clean extraction, import compressed encounters into one-line Adventure Nodes and key-beat bullets and discarded the rest — the full detail lived only in the import turn's context and was never persisted, so after compaction the DM ran encounters from memory of a summary. Import now writes **`encounters-full.md`** (verbatim read-aloud text, triggers, DCs, tactics, stat blocks, treasure — the encounter analogue of `npcs-full.md`) and copies the original source into the campaign's **`source/`** directory. A new play-loop rule (SKILL.md + SKILL-commands.md) requires the DM to re-read the full encounter entry before running it, mirroring the existing NPC rule, and to re-extract from `source/` rather than improvise when a detail is missing.
  - *Import process:* added guidance to dispatch **one chunk per background agent** — multi-chunk agents accumulate full chunk text and get compacted before finishing, silently dropping later-chunk details.
