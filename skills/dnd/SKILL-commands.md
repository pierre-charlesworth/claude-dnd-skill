...
5. Read state.md, world.md, npcs.md (index only), and all characters/*.md
   ...
   - **encounters-full.md (imported campaigns):** Do NOT load at start. **Before running any imported encounter or keyed area, read its full entry in `encounters-full.md` first — do not run it from the Adventure Node one-liner, the Campaign Arc beat, or memory.** The node/beat is only an index; the read-aloud text, triggers, DCs, tactics, stat blocks, and treasure live in the full entry, and these are exactly the details that get smoothed over when improvised from a summary. Do this proactively the moment the party commits toward an encounter, the same way you re-read an NPC entry before substantive dialogue. If the full entry looks thin or an important detail is missing, re-extract it from the preserved copy in `source/` rather than filling the gap from memory: use the encounter's recorded source anchor if it has one, otherwise look the heading up in `source/index.md` to find the chunk — then `python3 ${CLAUDE_SKILL_DIR}/scripts/import_campaign.py source/<file> --chunk N`. Do not scan chunks blindly when the index can point you straight to one.
...
2. **Preserve the source.** `mkdir -p ~/.claude/dnd/campaigns/<name>/source` and copy the original file in (`cp "<filepath>" ~/.claude/dnd/campaigns/<name>/source/`). This is the authoritative fallback: if any encounter detail is ever in doubt during play, the DM can re-extract it with `import_campaign.py` against the copy — even in a fresh container where the original path is gone. Then write the **source index** so re-extraction is a lookup, not a scan: `python3 ${CLAUDE_SKILL_DIR}/scripts/import_campaign.py ~/.claude/dnd/campaigns/<name>/source/<file> --index > ~/.claude/dnd/campaigns/<name>/source/index.md`. This maps every chunk to the headings it contains, so any detail — including unkeyed rooms, lore sidebars, and rules callouts that never got a per-encounter source anchor — can be found by heading and jumped to with `--chunk N`.
