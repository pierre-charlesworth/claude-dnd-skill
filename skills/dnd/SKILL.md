...
- **Before running any imported encounter or keyed area**, read its full entry in `encounters-full.md` if one exists. Do NOT run it from the Adventure Node one-liner, the Campaign Arc beat, or memory — those are only an index; the verbatim read-aloud text, triggers, DCs, tactics, stat blocks, and treasure live in the full entry, and these are exactly the details that get smoothed over when an encounter is improvised from a summary or from compacted context. Do this proactively the moment the party commits toward the encounter. If the full entry is thin or a detail is missing, re-extract it from the preserved copy in the campaign's `source/` directory rather than filling the gap from memory — use the encounter's source anchor, or `source/index.md` to look the heading up to a chunk, then read that chunk directly. Read the source yourself; never delegate source-consulting to a subagent, which would return a paraphrase and reintroduce exactly the lossy summary this rule exists to prevent.
